...
deathflash.style.background = '#000000';
...
score.update = function() {
    Maxscore = Math.pow(2, points);
    $("#Maxscore").text("最大值：" + Maxscore);
    for (var i = score.divs.length - 1; i >= 0; i--) { //49
        score.divs[i].innerHTML = points * points * points;
    }
...
var oef = function() {
    ...
    */
                ...
                if (game.ended) { // ???? you have to implement this. when the game is over, this is set to true.
                    //alert(time);//Game over points分数
                    clearTimeout(t);
					var inputscore=points*points*points;
					var inputgamephp = "php/inputgame.php?game=Flappy2048&score="+inputscore.toString()+"&time="+time.toString()+"&max="+Maxscore.toString();
					$.ajax({
							url:inputgamephp,
							type: "GET",
							dataType: "text",
							success: function(savegame){
								...
								//不完整
								if(isOK=="0") alert("战绩已保存！");
								if(isOK=="1") alert("未登录无法保存战绩！");
								if(isOK=="2") alert("连接服务器失败！");
							}
					});
...
function login(){
	window.location.href="../login/login.html?id=Flappy2048"; 
}
